...
const confirmAction = (message) =>
  new Promise((resolve) => {
    toast(
      ...
      {
        duration: Infinity,
        style: { padding: 0, background: 'transparent', boxShadow: 'none', maxWidth: '380px' },
      }
    );
  });
...
const CourseDetail = () => {
  ...
  useEffect(() => { fetchCourseData(); }, [id, navigate]);
    ...
    if (user?.role === 'student') {
      api.get('/enrollments/my')
        .then((res) => {
          ...
          setIsEnrolled(list.some((e) => String(e.course_id) === String(id)));
        })
        .catch(() => {});
    }
  }, [user, id]);
  ...
  const handleScheduleClass = async (e, moduleId) => {
    ...
    try {
      ...
      setModules(modules.map(mod => mod.id !== moduleId ? mod : {
        ...mod,
  ...
  const handleEnroll = async (method) => {
    ...
    try {
      if (method === 'razorpay') {
        navigate('/checkout/razorpay', { state: { courseId: id, courseName: course.title } });
        setEnrolling(null); return;
      } else if (method === 'stripe') {
        navigate('/checkout/stripe', { state: { courseId: id, courseName: course.title } });
        setEnrolling(null); return;
      }
    } catch (err) {
  ...
  const handleUpdateLesson = async (e, lessonId) => {
    ...
    try {
      ...
      setModules(modules.map(mod => !mod.lessons ? mod : {
        ...mod,
        lessons: mod.lessons.map(l => l.id === lessonId ? { ...l, ...editLessonForm } : l),
      }));
  ...
  const handleAddCourseFeature = () => {
    ...
    setCourseFormEdit((prev) => {
      ...
      if (prevFeatures.some(f => f.toLowerCase() === value.toLowerCase())) return prev;
  ...
  const handleRemoveCourseFeature = (featureToRemove) => {
    setCourseFormEdit((prev) => {
      if (!prev) return prev;
      return { ...prev, features: (prev.features || []).filter(f => f !== featureToRemove) };
    });
  };
  ...
  const handleUpdateCourse = async (e) => {
    ...
    if (trimmedTitle.length < COURSE_TITLE_MIN) {
      toast.error(`Course title must be at least ${COURSE_TITLE_MIN} characters.`); return;
    }
    if (trimmedTitle.length > COURSE_TITLE_MAX) {
      toast.error(`Course title must be ${COURSE_TITLE_MAX} characters or fewer.`); return;
    }
    const priceInr = Number(courseFormEdit.price_inr);
    if (!Number.isFinite(priceInr) || priceInr <= 0) {
      toast.error('Price (INR) must be greater than 0.'); return;
    }
  ...
  const handleEditCourseClick = () => {
    setCourseFormEdit({
      ...
      // pg driver returns DECIMAL as strings; convert for Zod validation
  ...
  const handleTogglePublish = async () => {
    try {
      ...
      toast.success(updated.is_published ? 'Course published — now visible to students.' : 'Course unpublished — hidden from students.');
    } catch (err) {
            ...
            {isTeacher && (
              <div className="flex flex-wrap gap-2 shrink-0 items-center">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2.5 px-4 py-2 rounded-lg border text-sm font-medium transition-all duration-200 select-none focus:outline-none whitespace-nowrap"
                ...
                >
                  <span
                    className="relative inline-flex h-4 w-7 flex-shrink-0 rounded-full transition-colors duration-300"
                    style={{ background: course.is_published ? '#22c55e' : '#475569' }}
                  >
                    <span className={`absolute top-0.5 left-0.5 inline-block h-3 w-3 rounded-full bg-white shadow-sm transition-transform duration-300 ${course.is_published ? 'translate-x-3' : 'translate-x-0'}`} />
                  </span>
          ...
          <div className="lg:col-span-2 space-y-8">
            {course.description && (
              <div>
                ...
                <div className="rounded-lg border border-neutral-200 bg-neutral-50/60 p-4 max-h-72 overflow-y-auto overflow-x-hidden">
                  <p className="text-neutral-600 leading-relaxed whitespace-pre-wrap break-words">{course.description}</p>
                </div>
              </div>
            )}
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                {isTeacher && (
                  <button onClick={() => setIsAddingModule(!isAddingModule)} className="btn-secondary text-sm py-2">
                ...
                <form onSubmit={handleCreateModule} className="card p-6 mb-8 bg-gradient-to-br from-primary-50/50 to-white border-2 border-primary-100 shadow-sm relative overflow-hidden">
                  ...
                  <div className="space-y-4 max-w-2xl">
                    <div>
                      ...
                      <input value={moduleForm.title} onChange={(e) => setModuleForm({ ...moduleForm, title: e.target.value })} className="input bg-white focus:ring-2 focus:ring-primary-500/20" placeholder="e.g., Week 1: Introductions and Greetings" required />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-1 block">Description (Optional)</label>
                      <textarea value={moduleForm.description} onChange={(e) => setModuleForm({ ...moduleForm, description: e.target.value })} className="input bg-white focus:ring-2 focus:ring-primary-500/20 resize-none h-24" placeholder="What key concepts will be covered in this module?" />
                    </div>
              ...
              <div className="space-y-3">
                ...
                {modules.map((mod, i) => (
                  <div key={mod.id} className="card p-0 overflow-hidden">
                    <div className="flex items-start gap-4 p-4 cursor-pointer hover:bg-neutral-50 transition-colors" onClick={() => toggleModule(mod.id)}>
                      ...
                      <div className="flex-1 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-neutral-400 font-medium">{mod.lessons?.length || 0} Lessons</span>
                          {isTeacher && (
                            <div className="flex items-center gap-2 border-l border-neutral-300 pl-3 ml-2" onClick={(e) => e.stopPropagation()}>
                              <button
                                onClick={(e) => { e.stopPropagation(); setEditingModuleId(mod.id); setEditModuleForm({ title: mod.title, description: mod.description || '' }); setExpandedModules(prev => ({ ...prev, [mod.id]: true })); }}
                                className="btn-secondary whitespace-nowrap !py-1.5 !px-3 shadow-none" title="Edit Module"
                              >
                              ...
                              <button
                                ...
                                className="btn-secondary whitespace-nowrap !py-1.5 !px-3 shadow-none !bg-red-50 !text-red-600 !border-red-200 hover:!bg-red-100" title="Delete Module"
                              >
                                <span className="flex items-center gap-1.5 text-xs">
                          ...
                          <div className="flex justify-end mb-4 pt-2">
                            <button onClick={() => setActiveModuleForClass(activeModuleForClass === mod.id ? null : mod.id)} className="text-sm text-primary-600 hover:text-primary-700 font-medium px-4 py-2 bg-primary-50 hover:bg-primary-100 rounded-lg transition-colors flex items-center gap-2">
                              {activeModuleForClass === mod.id ? (
                                <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>Cancel</>
                              ) : (
                                <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>Schedule Class</>
                              )}
                            </button>
                          </div>
                        )}
                                ...
                                {editingLessonId === lesson.id && isTeacher ? (
                                  ...
                                  <div className="flex justify-between items-center gap-4">
                                    <div className="flex-1">
                                      <p className="font-medium text-neutral-800 text-sm">{lesson.title}</p>
                                      <p className="text-xs text-neutral-500 flex gap-3 mt-1.5 flex-wrap">
                                        <span>📅 {new Date(lesson.scheduled_at).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}</span>
                                    ...
                                    <div className="flex gap-2 items-center flex-shrink-0">
                                      {(isEnrolled || isTeacher) && (
                                        new Date(lesson.scheduled_at) < new Date() ? (
                                          ...
                                          <button disabled className="btn-primary !bg-[#2D8CFF]/60 cursor-not-allowed py-1.5 px-4 text-xs flex items-center gap-1">
                                            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M4.5 4.5A2.5 2.5 0 002 7v10a2.5 2.5 0 002.5 2.5h11A2.5 2.5 0 0018 17V7a2.5 2.5 0 00-2.5-2.5h-11z" /><path d="M21.2 6.5l-2.7 2v7l2.7 2a1 1 0 001.5-.8v-9.4a1 1 0 00-1.5-.8z" /></svg>
                                            Link Pending
                                          </button>
                                        )
                                      ...
                                      {isTeacher && new Date(lesson.scheduled_at) >= new Date() && (
                                        <div className="flex gap-2 ml-3 pl-3 border-l border-neutral-300">
                                          <button
                                            onClick={() => { setEditingLessonId(lesson.id); setEditLessonForm({ title: lesson.title, description: lesson.description || '', zoom_link: lesson.zoom_link || '', scheduled_at: lesson.scheduled_at, duration_minutes: lesson.duration_minutes }); }}
                                            className="btn-secondary whitespace-nowrap !py-1 !px-2.5 shadow-none" title="Edit Lesson"
                                          >
                                            <span className="flex items-center gap-1.5 text-xs">
                                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                              Edit
                                          ...
                                          <button onClick={() => handleDeleteLesson(lesson.id)} className="btn-secondary whitespace-nowrap !py-1 !px-2.5 shadow-none !bg-red-50 !text-red-600 !border-red-200 hover:!bg-red-100" title="Delete Lesson">
                                            <span className="flex items-center gap-1.5 text-xs">
                                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                              Delete
                                            </span>
          ...
          <div className="lg:col-span-1">
            <div className="card p-6 sticky top-24">
              {course.thumbnail_url && (
                <img src={course.thumbnail_url} alt={course.title} className="w-full h-44 object-cover rounded-lg mb-5" />
              )}
              {!isEnrolled && !isTeacher && hasPricing && (
                <div className="mb-4">
                  {hasInrPrice && <p className="text-3xl font-bold text-neutral-900">₹{Number(course.price_inr).toLocaleString('en-IN')}</p>}
                  {hasUsdPrice && <p className="text-neutral-500 text-sm mt-0.5">${course.price_usd} USD{hasInrPrice ? ' for international students' : ''}</p>}
                </div>
              )}
              {!isEnrolled && !isTeacher && !hasPricing && (
              ...
              {isEnrolled ? (
                ...
                showPaymentOptions ? (
                  <div className="space-y-3 mt-4 border-t border-neutral-100 pt-4">
                    ...
                    {hasInrPrice && (
                      <button className="btn-primary w-full bg-[#3395FF] hover:bg-[#2084ea] disabled:opacity-50 !py-2.5" disabled={!!enrolling} onClick={() => handleEnroll('razorpay')}>
                        {enrolling === 'razorpay' ? <LoadingSpinner size="sm" /> : `Pay ₹${Number(course.price_inr).toLocaleString('en-IN')} (Razorpay)`}
                      </button>
                    )}
                    {hasUsdPrice && (
                      <button className="btn-secondary w-full border-[#635BFF] text-[#635BFF] hover:bg-slate-50 disabled:opacity-50 !py-2.5" disabled={!!enrolling} onClick={() => handleEnroll('stripe')}>
                        {enrolling === 'stripe' ? <LoadingSpinner size="sm" /> : `Pay ${course.price_usd} USD (Stripe)`}
                      </button>
                    )}
                  </div>
                ) : (
                  <button className="btn-primary w-full !py-3 font-semibold text-lg" onClick={() => setShowPaymentOptions(true)}>
                    Enroll Now
                  </button>
                )
              ) : null}
            ...
            <form onSubmit={handleUpdateCourse} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-6 overflow-y-auto flex-1 space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    value={courseFormEdit.description}
                    onChange={(e) => setCourseFormEdit({ ...courseFormEdit, description: e.target.value })}
                ...
                <div>
                  <label className="label">
                    Course Features <span className="text-neutral-400 font-normal">(optional)</span>
                  </label>
                  <div className="flex gap-2 mb-2">
                    <input
                      value={courseFeatureInput}
                      onChange={(e) => setCourseFeatureInput(e.target.value)}
                      onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddCourseFeature(); } }}
                      className="input bg-white flex-1"
                      placeholder="Add a feature (e.g. Weekly practice worksheets)"
                      maxLength={COURSE_FEATURE_TEXT_MAX}
                    />
                    <button type="button" onClick={handleAddCourseFeature} className="btn-secondary px-5">Add</button>
                  </div>
                  {(courseFormEdit.features || []).length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {(courseFormEdit.features || []).map((feature, i) => (
                        <span key={i} className="inline-flex items-center gap-1.5 bg-primary-50 text-primary-700 text-xs font-medium px-3 py-1.5 rounded-full border border-primary-100">
                          {feature}
                          <button type="button" onClick={() => handleRemoveCourseFeature(feature)} className="text-primary-400 hover:text-red-500 transition-colors ml-0.5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={courseFormEdit.level} onChange={(e) => setCourseFormEdit({ ...courseFormEdit, level: e.target.value })} className="input bg-white">
                  ...
                  <div>
                    ...
                    <input type="number" min={1} value={courseFormEdit.duration_weeks} onChange={(e) => setCourseFormEdit({ ...courseFormEdit, duration_weeks: Number(e.target.value) })} className="input bg-white" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" min={1} value={courseFormEdit.price_inr} onChange={(e) => setCourseFormEdit({ ...courseFormEdit, price_inr: Number(e.target.value) })} className="input bg-white" required />
                  </div>
                  <div>
                    <label className="label">Price (USD)</label>
                    <input type="number" min={0} value={courseFormEdit.price_usd} onChange={(e) => setCourseFormEdit({ ...courseFormEdit, price_usd: Number(e.target.value) })} className="input bg-white" />
                  </div>
                </div>

                {/* Thumbnail URL + file picker on separate row */}
                <div>
                  ...
                  <div className="mt-2">
                    <input
                      type="file"
                      accept="image/*"
                      className="text-sm text-neutral-500 file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-neutral-300 file:text-sm file:bg-white file:text-neutral-700 file:cursor-pointer hover:file:bg-neutral-50"
                      onChange={(e) => handleThumbnailUpload(e.target.files[0])}
                      disabled={uploadingThumbnail}
                    />
                    {uploadingThumbnail && <span className="text-xs text-neutral-400 ml-2">Uploading…</span>}
                  </div>
                  {courseFormEdit.thumbnail_url && (
                    <img src={courseFormEdit.thumbnail_url} alt="Preview" className="mt-2 h-20 rounded-lg object-cover" />
                  )}
                </div>
              ...
              {/* Footer */}
              <div className="px-6 py-4 border-t border-neutral-100 flex justify-end gap-3 bg-neutral-50/50 flex-shrink-0 rounded-b-2xl">
                <button type="button" onClick={() => setEditingCourse(false)} className="btn-secondary">Cancel</button>
